...
const UID = 'api::volunteer-page.volunteer-page';
const SEED_PATH = path.join(__dirname, 'volunteer-page-seed.json');
...
function readSeedFile() {
  ...
  if (!data?.locales) {
    throw new Error('Invalid volunteer-page seed file: missing "locales" object.');
  }

  for (const locale of REQUIRED_LOCALES) {
    if (!data.locales[locale]) {
      throw new Error(`Invalid volunteer-page seed file: missing "${locale}" locale payload.`);
    }
  }
...
function normalizePayload(localePayload, locale) {
  return {
    locale,
    ...localePayload,
    benefits: (localePayload.benefits || []).map(benefit => ({
      title: benefit.title,
    })),
...
async function seedVolunteerPage(strapi) {
  ...
  console.log(`Volunteer page seed complete. uk id=${ukEntry.id}, en id=${enEntry.id}`);
}
