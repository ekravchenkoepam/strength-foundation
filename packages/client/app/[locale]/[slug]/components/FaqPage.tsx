...
const contacts = [
  ...
  {
    title:
      'Або ж заповніть форму за посиланням. Ми зв’яжемось із вами протягом 1–2 днів.',
    highlight: '',
    href: '#',
    ...
    <div className="w-full text-[16px] text-[var(--color-dark)]">
      <section className="-mx-[50px] bg-[var(--color-light)] px-[52px] pt-[32px] pb-[90px]">
        <div className="mx-auto flex w-full flex-col items-center text-center">
          ...
          <div className="max-w-[760px] text-[16px] text-left leading-7 text-[var(--black-80)]">
            Наш фонд підтримує родини військовополонених та зниклих безвісти. Ми надаємо
            юридичні консультації, психологічну підтримку, а також проводимо зустрічі,
            заходи й активності для родин та дітей. Якщо вам потрібна допомога —
            звертайтесь у зручний для вас спосіб.
          </div>
        </div>
      </section>
      ...
      <section className="bg-[var(--green-100)] bg-[url('/images/asphalt-bg.png')] bg-cover bg-center py-[100px] px-[50px] text-[var(--color-light)]">
        <div className="mx-auto">
          <div className="h2 text-center text-[var(--color-light)] pb-[80px]">
          ...
          <div className="grid grid-cols-1 gap-[32px] md:grid-cols-2">
            {contacts.map((item, index) => (
              <div
                key={item.title}
                className="flex flex-col gap-[24px] rounded-[14px] border border-[rgba(255,255,255,0.25)] bg-[rgba(255,255,255,0.04)] p-[24px] text-[24px]"
              >
               <div className="">
                ...
                <div className="leading-6 text-[var(--color-light)]">
                  {item.title}{' '}
                  <a
                    className="underline decoration-[var(--yellow-100)] underline-offset-4"
                    href={item.href}
                  >
                    {item.highlight}
                  </a>
                  ...
                  <Collapse
                    ...
                    trigger={
                      <div className="flex w-full items-center justify-between gap-4 text-left text-[16px] font-semibold">
                        <span>{getQuestionIndex(index)} {item.question}</span>
                      </div>
                    }
                    className="rounded-[12px]"
                    triggerClassName="text-[16px] font-semibold data-[state=open]:border-b-2 data-[state=open]:border-[#EFCB4C] data-[state=open]:rounded-b-none"
